...
int main(int argc, char* argv[]) {
	...
	// Grabs the optional second argument
	if (argc > 2) {
		for (int i = 2; i < argc; ++i) {
			std::string optArg = argv[i];
			if (optArg == "-all") printAll = true;
			else if (optArg == "-bin") printBin = true;
			else if (optArg == "-dec") printDec = true;
			else if (optArg == "-hex") printHex = true;
			else if (optArg == "-oct") printOct = true;
			else {
				std::cout << "UNRECOGNIZED FLAG -v" << std::endl;
				return -1;
			}
		}
	}
